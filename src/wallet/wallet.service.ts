...
import { CreditLogsResponseDto, IncreaseWalletResponseDto, TotalCreditResponseDto } from "./wallet.dto";
...
export class WalletService {
    ...
    async totalCredit(userId: string): Promise<TotalCreditResponseDto> {
        ...
        const transactions = await this.walletTransactionRepository.getTransactionsByUserId(userId) ?? [];

        return {
            total_credit: transactions.reduce((sum, tx) => Number(sum) + Number(tx.amount), 0)
        }
    }
    ...
    async increaseWallet(userId: string, amount: number): Promise<IncreaseWalletResponseDto> {
        ...
        try {
            const transaction = await this.walletTransactionRepository.createTransaction({
